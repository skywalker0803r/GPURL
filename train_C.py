...
在「場景 C」下接續「場景 B」的模型 (modelA_B.zip)，繼續訓練。
需確保:
  - env.py, custom_callback.py, monitor.py, reward.py 與 train.py 同一層
  - modelA_B.zip 為上次訓練完成後保存的模型
"""
...
def main():
    ...
    # 1. 準備環境
    # =============================================
    # 若「場景 C」需要在程式中動態切換風扇或散熱條件，可在這裡做:
    
    env = GPUEnv(gpu_id=0, step_time=2.0)

    # =============================================
    # 2. 載入之前的模型 (modelA_B.zip)
    # =============================================
    # 指定新的環境給這個模型
    model = PPO.load("modelA_B.zip", env=env)
    print("載入 modelA_B.zip 成功，準備繼續在『場景 C』訓練...")
    ...
    # 5. 存檔 (modelA_B_C.zip)
    # =============================================
    model.save("modelA_B_C.zip")
    print("場景 C 訓練完成，已保存為 modelA_B_C.zip")
